...
class Server:
    ...
    async def _install(self, cb: InternalCB):
        ...
        if is_windows and (self._comfy_dir is None or no_python):
        ...
        elif not is_windows and (self._comfy_dir is None or self._pip_cmd is None):
    ...
    async def _install_comfy(self, network: QNetworkAccessManager, cb: InternalCB):
        ...
        await _execute_process("PyTorch", [self._pip_cmd, *torch_args], self._comfy_dir, cb)
        ...
        if self.backend is ServerBackend.directml:
            await _execute_process(  # for some reason this must come AFTER ComfyUI requirements
                "PyTorch", [self._pip_cmd, "install", "torch-directml"], self._comfy_dir, cb
            )
